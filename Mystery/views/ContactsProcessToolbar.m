...
- (void)softKeyboard:(UISoftKeyboard *)pSoftKeyboard didSelectCellAtIndexPath:(NSIndexPath *)indexPath{    
    ...
    // numeric
    if (indexPath.skb_cell < 3 && (indexPath.skb_row < 3 || (indexPath.skb_row == 3 && indexPath.skb_cell == 1))) {
        // set inputDiaplayLabel text for user input with custom softKeyboard
        _mUserInputTextField.text = [NSString stringWithFormat:@"%@%@", !_mUserInputTextField.text ? @"" : _mUserInputTextField.text, [pSoftKeyboard cellForRowAtIndexPath:indexPath].coreData.stringValue];
        // manual call method:(void)userInputTextDidChanged:
        [self userInputTextDidChanged:_mUserInputTextField];
    }
    // functionality
    else {
        if (indexPath.skb_row == 0 && indexPath.skb_cell == 3) {
            // call
            NSLog(@"not implement call");
        }
        else if (indexPath.skb_row == 2 && indexPath.skb_cell == 3) {
            // batchProc
            NSLog(@"not implement batchProc");
        }
        else if (indexPath.skb_row == 3 && indexPath.skb_cell == 0) {
            // add
            NSLog(@"not implement add");
        }
